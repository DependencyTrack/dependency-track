...
```shell
mvn clean package -P clean-exclude-wars -P embedded-jetty -DskipTests -Dlogback.configuration.file=src/main/docker/logback.xml
```

Build an executable JAR that contains both API server and frontend (aka "bundled" distribution):

```shell
mvn clean package -P clean-exclude-wars -P embedded-jetty -P bundle-ui -DskipTests -Dlogback.configuration.file=src/main/docker/logback.xml
```
...
```shell
mvn jetty:run -Dlogback.configurationFile=src/main/docker/logback.xml
```
...
```shell
mvn jetty:run -P h2-console -Dlogback.configurationFile=src/main/docker/logback.xml
```
...
```shell
mvn -Dtest=ComposerMetaAnalyzerTest verify
```
