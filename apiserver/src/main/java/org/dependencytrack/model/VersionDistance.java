...
public class VersionDistance implements Comparable<VersionDistance>, Serializable {
    ...
    protected static final Pattern VERSION_PATTERN = Pattern.compile(
            "^(?:(?<" + GROUP_EPOCH + ">.*):)?"
                    + // Optional epoch part: number before the first : sign. Match any characters here so we can fail
                    // on incorrect values
                    "v?(?<"
                    + GROUP_MAJOR + ">\\d+[a-z]*)?(?:\\.(?<" + GROUP_MINOR + ">\\d+[a-z]*))?(?:\\.(?<" + GROUP_PATCH
                    + ">\\d+[a-z]*))?"
                    + // version part, at least major version (numeric), optinali minor (numeric) or patch (numeric)
                    // version. Ignore the rest
