...
public class DependencyGraphResource extends AbstractApiResource {
    ...
    public Response getComponentsAndServicesByProjectUuid(
        ...
        try (QueryManager qm = new QueryManager(getAlpineRequest())) {
            ...
            if (directDependenciesJSON != null) {
                final List<DependencyGraphResponse> response =
                        getDependencyGraphFromDirectDependenciesJSON(qm, directDependenciesJSON);
    ...
    @PermissionRequired(Permissions.Constants.VIEW_PORTFOLIO)
    public Response getComponentsAndServicesByComponentUuid(
            @Parameter(
     ...
     * @since 4.9.0
     */
    private List<DependencyGraphResponse> getDependencyGraphFromDirectDependenciesJSON(
            final QueryManager qm, final String directDependenciesJSON) {
        ...
        final List<UUID> uuids = directDependencies.stream()
                .map(directDependency -> directDependency.asJsonObject().getString("uuid"))
                .map(UUID::fromString)
                .toList();
        ...
        if (!purlPackages.isEmpty()) {
            final List<PackageMetadata> packageMetadataList =
                    withJdbiHandle(handle -> new PackageMetadataDao(handle).getAll(purlPackages));
