...
public class FindingResource extends AbstractV1ApiResource {
    ...
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "200",
                        description =
                                "A list of all findings for a specific project, or a SARIF file. SARIF responses carry no count headers.",
                        headers = {
                            @Header(
                                    name = TOTAL_COUNT_HEADER,
                                    description =
                                            "The number of findings, exact or a lower bound. See `X-Total-Count-Type`.",
    ...
    @PaginatedApi
    @Parameter(name = "searchText", in = ParameterIn.QUERY, description = """
    ...
    public Response getFindingsByProject(
            @Parameter(
                    ...
                    @ValidUuid
                    String uuid,
            @Parameter(description = "Optionally includes suppressed findings") @QueryParam("suppressed")
                    boolean suppressed,
            ...
            @Parameter(description = "Whether to include only projects with existing analysis.")
                    @QueryParam("hasAnalysis")
                    final Boolean hasAnalysis,
            @Parameter(description = "Filter EPSS score from this value (inclusive)") @QueryParam("epssFrom")
                    final BigDecimal epssFrom,
            @Parameter(description = "Filter EPSS score to this value (inclusive)") @QueryParam("epssTo")
                    final BigDecimal epssTo,
            @Parameter(
                            description =
                                    "Filter by known exploited vulnerability (KEV) status: omit for any, true for KEVs only, false to exclude KEVs")
                    @QueryParam("isKev")
                    final Boolean isKev,
            @Parameter(
                            description =
                    ...
                    @DefaultValue("EXACT")
                    final TotalCountMode totalCount) {
        try (QueryManager qm = new QueryManager(getAlpineRequest())) {
            final Project project = qm.getObjectByUuid(Project.class, uuid);
            if (project != null) {
                requireAccess(qm, project);
                final String rawFilter = getAlpineRequest().getFilter();
                final String searchText = (rawFilter != null && !rawFilter.isBlank())
                ...
                if (acceptHeader != null && acceptHeader.contains(MEDIA_TYPE_SARIF_JSON)) {
                    ...
                    final List<FindingDao.FindingRow> rows = withJdbiHandle(
                            getAlpineRequest(),
                    ...
                    try {
                        ...
                        return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                                .entity("An error occurred while generating SARIF file")
                                .build();
                    }
                }
                ...
                return withTotalCountHeaders(
                                Response.ok(mapComponentLatestVersion(
                                        page.items().stream().map(Finding::new).toList())),
                                page.totalCount())
                        .build();
            } else {
    ...
    @PermissionRequired(Permissions.Constants.VIEW_VULNERABILITY)
    public Response exportFindingsByProject(
            @Parameter(
                            description = "The UUID of the project",
                            schema = @Schema(type = "string", format = "uuid"),
                            required = true)
        ...
        try (QueryManager qm = new QueryManager(getAlpineRequest())) {
            ...
            if (project != null) {
                requireAccess(qm, project);
                final List<Finding> findings = withJdbiHandle(
                        getAlpineRequest(),
                        handle -> handle.attach(FindingDao.class).getFindings(project.getId(), false));
    ...
    public Response analyzeProject(
        ...
        final UUID runId = dexEngine.createRun(new CreateWorkflowRunRequest<>(AnalyzeProjectWorkflow.class)
        ...
        useJdbiTransaction(
                handle -> handle.attach(ProjectLastAnalysisDao.class).recordAttempt(projectUuid, Instant.now()));
    ...
    @PaginatedApi
    @PermissionRequired(Permissions.Constants.VIEW_VULNERABILITY)
    public Response getAllFindings(
            @Parameter(description = "Show inactive projects") @QueryParam("showInactive") boolean showInactive,
            @Parameter(description = "Show suppressed findings") @QueryParam("showSuppressed") boolean showSuppressed,
            @Parameter(description = "Filter by severity") @QueryParam("severity") String severity,
            @Parameter(description = "Filter by analysis status") @QueryParam("analysisStatus") String analysisStatus,
            @Parameter(description = "Filter by vendor response") @QueryParam("vendorResponse") String vendorResponse,
            @Parameter(description = "Filter published from this date") @QueryParam("publishDateFrom")
                    String publishDateFrom,
            @Parameter(description = "Filter published to this date") @QueryParam("publishDateTo") String publishDateTo,
            @Parameter(description = "Filter attributed on from this date") @QueryParam("attributedOnDateFrom")
                    String attributedOnDateFrom,
            @Parameter(description = "Filter attributed on to this date") @QueryParam("attributedOnDateTo")
                    String attributedOnDateTo,
            @Parameter(description = "Filter the text input in these fields") @QueryParam("textSearchField")
                    String textSearchField,
            @Parameter(description = "Filter by this text input") @QueryParam("textSearchInput") String textSearchInput,
            @Parameter(description = "Filter CVSSv2 from this value") @QueryParam("cvssv2From") String cvssv2From,
            @Parameter(description = "Filter CVSSv2 from this Value") @QueryParam("cvssv2To") String cvssv2To,
            @Parameter(description = "Filter CVSSv3 from this value") @QueryParam("cvssv3From") String cvssv3From,
            @Parameter(description = "Filter CVSSv3 from this Value") @QueryParam("cvssv3To") String cvssv3To,
            @Parameter(description = "Filter CVSSv4 from this value") @QueryParam("cvssv4From") String cvssv4From,
            @Parameter(description = "Filter CVSSv4 to this value") @QueryParam("cvssv4To") String cvssv4To,
            @Parameter(description = "Filter EPSS from this value") @QueryParam("epssFrom") String epssFrom,
            @Parameter(description = "Filter EPSS to this value") @QueryParam("epssTo") String epssTo,
            @Parameter(description = "Filter EPSS Percentile from this value") @QueryParam("epssPercentileFrom")
                    String epssPercentileFrom,
            @Parameter(description = "Filter EPSS Percentile to this value") @QueryParam("epssPercentileTo")
                    String epssPercentileTo,
            @Parameter(
                            description =
                                    "Filter by known exploited vulnerability (KEV) status: omit for any, true for KEVs only, false to exclude KEVs")
                    @QueryParam("isKev")
        ...
        final Page<FindingDao.FindingRow> page = withJdbiHandle(
                ...
                handle -> handle.attach(FindingDao.class)
                        .getAllFindings(
                                filters, showSuppressed, showInactive, orderBy, totalCountThreshold(totalCount)));
    ...
    public Response getAllFindings(
            ...
            @Parameter(description = "Filter by this text input") @QueryParam("textSearchInput") String textSearchInput,
            @Parameter(description = "Filter CVSSv2 from this value") @QueryParam("cvssv2From") String cvssv2From,
            @Parameter(description = "Filter CVSSv2 to this value") @QueryParam("cvssv2To") String cvssv2To,
            @Parameter(description = "Filter CVSSv3 from this value") @QueryParam("cvssv3From") String cvssv3From,
            @Parameter(description = "Filter CVSSv3 to this value") @QueryParam("cvssv3To") String cvssv3To,
            @Parameter(description = "Filter CVSSv4 from this value") @QueryParam("cvssv4From") String cvssv4From,
            @Parameter(description = "Filter CVSSv4 to this value") @QueryParam("cvssv4To") String cvssv4To,
            @Parameter(description = "Filter EPSS from this value") @QueryParam("epssFrom") String epssFrom,
            @Parameter(description = "Filter EPSS to this value") @QueryParam("epssTo") String epssTo,
            ...
            @Parameter(description = "Filter EPSS Percentile to this value") @QueryParam("epssPercentileTo")
                    String epssPercentileTo,
            @Parameter(description = "Filter occurrences in projects from this value") @QueryParam("occurrencesFrom")
                    String occurrencesFrom,
            @Parameter(description = "Filter occurrences in projects to this value") @QueryParam("occurrencesTo")
                    String occurrencesTo,
            @Parameter(
                            description =
                                    "Filter by known exploited vulnerability (KEV) status: omit for any, true for KEVs only, false to exclude KEVs")
                    @QueryParam("isKev")
        ...
        final Page<FindingDao.GroupedFindingRow> page = withJdbiHandle(
                ...
                handle -> handle.attach(FindingDao.class)
                        .getGroupedFindings(
                                filters, showInactive, /* boundedTotalCount */ totalCount == TotalCountMode.BOUNDED));

        return withTotalCountHeaders(
                        Response.ok(
                                page.items().stream().map(GroupedFinding::new).toList()),
                        page.totalCount())
                .build();
    }
    ...
    public static List<Finding> mapComponentLatestVersion(List<Finding> findingList) {
        final Map<String, List<Finding>> findingsByPurlPackage = findingList.stream()
                ...
                .map(finding -> {
                    final PackageURL purl =
                            PurlUtil.silentPurl((String) finding.getComponent().get("purl"));
                    if (purl == null) {
                        return null;
                    }
        ...
        final List<PackageMetadata> packageMetadataList =
                withJdbiHandle(handle -> new PackageMetadataDao(handle).getAll(findingsByPurlPackage.keySet()));
