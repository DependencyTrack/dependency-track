...
public class LdapResource extends AbstractApiResource {
    ...
    @Operation(summary = "Returns the DNs of all accessible groups within the directory", description = """
                    <p>
                ...
                @ApiResponse(
                        ...
                        headers =
                                @Header(
                                        ...
                                        description =
                                                "The total number of ldap groups that match the specified search criteria",
    ...
    public Response retrieveLdapGroups() {
        ...
        try {
            ...
            LOGGER.warn(
                    "The LDAP server did not return results from the specified search criteria as the result list would have exceeded the size limit specified by the LDAP server");
            return Response.status(Response.Status.NO_CONTENT).build();
        } catch (NamingException e) {
            LOGGER.error(
                    "An error occurred attempting to retrieve a list of groups from the configured LDAP server", e);
    ...
    public Response addMapping(MappedLdapGroupRequest request) {
        ...
        failOnValidationError(validator.validateProperty(request, "team"), validator.validateProperty(request, "dn"));
        try (QueryManager qm = new QueryManager(getAlpineRequest())) {
