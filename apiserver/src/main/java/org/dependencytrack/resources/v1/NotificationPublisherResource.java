...
public class NotificationPublisherResource extends AbstractApiResource {
                                        ...
                                        array =
                                                @ArraySchema(
                                                        schema =
                                                                @Schema(
                                                                        implementation =
                                                                                NotificationPublisherResponse.class)))),
    ...
    public Response getAllNotificationPublishers() {
        try (final var qm = new QueryManager(getAlpineRequest())) {
            final List<NotificationPublisherResponse> publishers = qm.getAllNotificationPublishers().stream()
    ...
    public Response updateNotificationPublisher(UpdateNotificationPublisherRequest request) {
            ...
            updatedPublisher = qm.callInTransaction(() -> {
                ...
                if (!request.name().equals(publisher.getName())) {
                    ...
                    if (conflictingPublisher != null) {
                        throw new ClientErrorException(Response.status(Response.Status.CONFLICT)
                                .entity("An existing publisher with the name '" + conflictingPublisher.getName()
                                        + "' already exist")
                                .build());
                    }
                }
        ...
        return Response.ok(NotificationPublisherResponse.of(updatedPublisher)).build();
    }
    ...
    public Response deleteNotificationPublisher(
            @Parameter(
                            ...
                            required = true)
                    @PathParam("notificationPublisherUuid")
                    @ValidUuid
                    String uuid) {
        try (final var qm = new QueryManager(getAlpineRequest())) {
    ...
    public Response getNotificationPublisherConfigSchema(@PathParam("uuid") UUID uuid) {
        ...
        final NotificationPublisherFactory extensionFactory = requireExtensionExists(extensionName);
        ...
        try (final var qm = new QueryManager(getAlpineRequest())) {
            ...
            for (final var notificationGroup : rule.getNotifyOn()) {
                ...
                final Notification notification =
                        buildTestNotification(convert(rule.getScope()), group, convert(rule.getNotificationLevel()));
                ...
                createRunRequests.add(new CreateWorkflowRunRequest<>(PublishNotificationWorkflow.class)
                        .withWorkflowInstanceId(
                                "publish-test-notification:%s:%s".formatted(rule.getUuid(), notificationGroup))
    ...
    private @Nullable Notification buildTestNotification(Scope scope, Group group, Level ruleLevel) {
        final List<Level> levelsToTry =
                switch (ruleLevel) {
                    case LEVEL_INFORMATIONAL ->
                        List.of(Level.LEVEL_INFORMATIONAL, Level.LEVEL_WARNING, Level.LEVEL_ERROR);
