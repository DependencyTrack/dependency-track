...
public class NotificationRuleResource extends AbstractApiResource {
                ...
                @ApiResponse(
                        ...
                        content =
                                @Content(
                                        array =
                                                @ArraySchema(
                                                        schema = @Schema(implementation = NotificationRule.class)))),
    ...
    public Response getAllNotificationRules(
            @Parameter(description = "The notification trigger type to filter on") @QueryParam("triggerType")
                    final NotificationTriggerType triggerTypeFilter) {
        try (final var qm = new QueryManager(getAlpineRequest())) {
            ...
            final Set<UUID> accessibleProjectUuids = filterAccessibleProjects(rules.stream()
    ...
    public Response createNotificationRule(@Valid CreateNotificationRuleRequest request) {
        ...
        try (final var qm = new QueryManager(getAlpineRequest())) {
            createdRule = qm.callInTransaction(() -> {
                ...
                if (request.publisher() != null) {
                    publisher = qm.getObjectByUuid(
                            NotificationPublisher.class, request.publisher().uuid());
                }
                if (publisher == null) {
        ...
        return Response.status(Response.Status.CREATED).entity(createdRule).build();
    }
    ...
    public Response createScheduledNotificationRule(@Valid CreateScheduledNotificationRuleRequest request) {
        ...
        try (final var qm = new QueryManager(getAlpineRequest())) {
            createdRule = qm.callInTransaction(() -> {
                ...
                final NotificationRule rule =
                        qm.createScheduledNotificationRule(request.name(), request.scope(), request.level(), publisher);
        ...
        try (final var qm = new QueryManager(getAlpineRequest())) {
            final NotificationRule updatedRule = qm.callInTransaction(() -> {
                ...
                if (rule.getScope() != NotificationScope.PORTFOLIO) {
                    throw new ClientErrorException(Response.status(Response.Status.NOT_ACCEPTABLE)
