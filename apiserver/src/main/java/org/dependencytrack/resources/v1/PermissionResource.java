...
public class PermissionResource extends AbstractApiResource {
    ...
    public Response addPermissionToUser(
            ...
            @Parameter(description = "A valid permission", required = true) @PathParam("permission")
        ...
        try (QueryManager qm = new QueryManager(getAlpineRequest())) {
            return qm.callInTransaction(() -> {
                ...
                if (permission == null) {
                    return Response.status(Response.Status.NOT_FOUND)
                            .entity("The permission could not be found.")
                            .build();
                }
                ...
                if (permissions != null && !permissions.contains(permission)) {
                    ...
                    super.logSecurityEvent(
                            ...
                            "Added permission for user: " + principal.getUsername() + " / permission: "
                                    + permission.getName());
            ...
            return qm.callInTransaction(() -> {
                ...
                if (permissions != null && permissions.contains(permission)) {
                    ...
                    super.logSecurityEvent(
                            LOGGER,
                            SecurityMarkers.SECURITY_AUDIT,
                            "Removed permission for user: " + principal.getUsername() + " / permission: "
                                    + permission.getName());
                    return Response.ok(principal).build();
                }
                return Response.status(Response.Status.NOT_MODIFIED).build();
            ...
            return qm.callInTransaction(() -> {
                ...
                if (permissions != null && permissions.contains(permission)) {
                    ...
                    super.logSecurityEvent(
                            LOGGER,
                            SecurityMarkers.SECURITY_AUDIT,
                            "Removed permission for team: " + team.getName() + " / permission: "
                                    + permission.getName());
                    return Response.ok(team).build();
                }
                return Response.status(Response.Status.NOT_MODIFIED).build();
    ...
    public Response setUserPermissions(
        ...
        try (QueryManager qm = new QueryManager(getAlpineRequest())) {
            return qm.callInTransaction(() -> {
                ...
                final Query<Permission> query = qm.getPersistenceManager()
                        .newQuery(Permission.class)
        ...
        try (QueryManager qm = new QueryManager(getAlpineRequest())) {
            return qm.callInTransaction(() -> {
                ...
                super.logSecurityEvent(
                        LOGGER,
                        SecurityMarkers.SECURITY_AUDIT,
                        "Set permissions for team: %s / permissions: %s".formatted(team.getName(), permissionNames));
                return Response.ok(team).build();
            });
        }
    }
