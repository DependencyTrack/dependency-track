...
public class PolicyResource extends AbstractApiResource {
    ...
    public Response getPolicies() {
        try (QueryManager qm = new QueryManager(getAlpineRequest())) {
            ...
            final Set<UUID> accessibleProjectUuids = filterAccessibleProjects(policies.stream()
    ...
    public Response createPolicy(Policy jsonPolicy) {
        ...
        failOnValidationError(validator.validateProperty(jsonPolicy, "name"));
            ...
            return qm.callInTransaction(() -> {
                ...
                if (policy == null) {
                    ...
                    policy = qm.createPolicy(
                            ...
                            operator,
                            violationState,
                            jsonPolicy.isOnlyLatestProjectVersion());
                    return Response.status(Response.Status.CREATED)
                            .entity(policy)
                            .build();
                } else {
                    return Response.status(Response.Status.CONFLICT)
                            .entity("A policy with the specified name already exists.")
                            .build();
                }
            });
        }
