...
public class ServiceResource extends AbstractApiResource {
                ...
                @ApiResponse(
                        ...
                        content =
                                @Content(
                                        array =
                                                @ArraySchema(
                                                        schema = @Schema(implementation = ServiceComponent.class)))),
    ...
    public Response getServiceByUuid(
        ...
        try (QueryManager qm = new QueryManager(getAlpineRequest())) {
            ...
            if (service != null) {
                ...
                final ServiceComponent detachedService = qm.detach(
                        ServiceComponent.class,
                        service.getId()); // TODO: Force project to be loaded. It should be anyway, but JDO seems to be
                // having issues here.
                return Response.ok(detachedService).build();
            } else {
                return Response.status(Response.Status.NOT_FOUND)
                        .entity("The service could not be found.")
                        .build();
            }
        }
    }
            ...
            return qm.callInTransaction(() -> {
                ...
                if (project.getCollectionLogic() != null) {
                    return Response.status(Response.Status.BAD_REQUEST)
                            .entity("A collection project cannot contain services.")
                            .build();
                }
        ...
        try (QueryManager qm = new QueryManager(getAlpineRequest())) {
            return qm.callInTransaction(() -> {
                ...
                if (service != null) {
                    ...
                    return Response.status(Response.Status.NOT_FOUND)
                            .entity("The UUID of the service could not be found.")
                            .build();
                }
            });
        }
        ...
        try (QueryManager qm = new QueryManager(getAlpineRequest())) {
            return qm.callInTransaction(() -> {
                final ServiceComponent service =
                        qm.getObjectByUuid(ServiceComponent.class, uuid, ServiceComponent.FetchGroup.ALL.name());
                if (service != null) {
                    requireAccess(qm, service.getProject());
                    qm.delete(service);
