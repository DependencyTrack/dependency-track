...
public class UserResource extends AbstractApiResource {
    ...
    public Response validateOidcAccessToken(
            @Parameter(description = "An OAuth2 access token", required = true) @FormParam("idToken")
        ...
        try (final QueryManager qm = new QueryManager(getAlpineRequest())) {
            return qm.callInTransaction(() -> {
                try {
                    ...
                    super.logSecurityEvent(
                            LOGGER, SecurityMarkers.SECURITY_FAILURE, "Unauthorized OpenID Connect login attempt");
                    if (AlpineAuthenticationException.CauseType.SUSPENDED == e.getCauseType()
                            || AlpineAuthenticationException.CauseType.UNMAPPED_ACCOUNT == e.getCauseType()) {
                        return Response.status(Response.Status.FORBIDDEN)
                                .entity(e.getCauseType().name())
    ...
    @AuthenticationNotRequired
    public Response forceChangePassword(
            ...
            return qm.callInTransaction(() -> {
                try {
                    ...
                    if (principal.get() instanceof ManagedUser) {
                        ...
                        if (StringUtils.isNotBlank(newPassword)
                            ...
                            if (PasswordService.matches(newPassword.toCharArray(), user)) {
                                ...
                                return Response.status(Response.Status.NOT_ACCEPTABLE)
                                        .entity("Existing password is the same as new password. Password not changed.")
                                        .build();
                            } else {
                            ...
                            return Response.status(Response.Status.NOT_ACCEPTABLE)
                                    .entity("The passwords do not match. Password not changed.")
                                    .build();
                        }
                    } else {
                        super.logSecurityEvent(
                        ...
                        return Response.status(Response.Status.NOT_ACCEPTABLE)
                                .entity(
                                        "Changing passwords for non-managed users is not forbidden. Password not changed.")
                                .build();
                    }
                } catch (AlpineAuthenticationException e) {
                    if (AlpineAuthenticationException.CauseType.SUSPENDED == e.getCauseType()
            ...
            return qm.callInTransaction(() -> {
                ...
                if (user == null) {
                    ...
                    new JdoNotificationEmitter(qm)
                            .emit(createUserCreatedNotification(NotificationModelConverter.convert(user)));
    ...
    public Response createManagedUser(ManagedUser jsonUser) {
        try (QueryManager qm = new QueryManager(getAlpineRequest())) {
            return qm.callInTransaction(() -> {
                ...
                if (StringUtils.isBlank(jsonUser.getNewPassword())
                        || StringUtils.isBlank(jsonUser.getConfirmPassword())) {
                    return Response.status(Response.Status.BAD_REQUEST)
                ...
                if (user == null) {
                    user = qm.createManagedUser(
                            ...
                            jsonUser.getFullname(),
                            jsonUser.getEmail(),
                            String.valueOf(PasswordService.createHash(
                                    jsonUser.getNewPassword().toCharArray())),
                            jsonUser.isForcePasswordChange(),
        ...
        try (QueryManager qm = new QueryManager(getAlpineRequest())) {
            return qm.callInTransaction(() -> {
                ...
                if (user != null) {
                    ...
                    super.logSecurityEvent(
                            LOGGER, SecurityMarkers.SECURITY_AUDIT, "Managed user updated: " + jsonUser.getUsername());
                    return Response.ok(user).build();
                } else {
                    return Response.status(Response.Status.NOT_FOUND)
                            .entity("The user could not be found.")
            ...
            return qm.callInTransaction(() -> {
                ...
                if (modified) {
                    super.logSecurityEvent(
                            LOGGER,
                            SecurityMarkers.SECURITY_AUDIT,
                            "Added team membership for: " + principal.getUsername() + " / team: " + team.getName());
                    return Response.ok(principal).build();
                } else {
                    return Response.status(Response.Status.NOT_MODIFIED)
                            .entity("The user is already a member of the specified team.")
                            .build();
                }
            });
        }
            ...
            return qm.callInTransaction(() -> {
                ...
                if (modified) {
                    super.logSecurityEvent(
                            LOGGER,
                            SecurityMarkers.SECURITY_AUDIT,
                            "Removed team membership for: " + principal.getUsername() + " / team: " + team.getName());
                    return Response.ok(principal).build();
                } else {
                    return Response.status(Response.Status.NOT_MODIFIED)
    ...
    public Response setUserTeams(
            @Parameter(description = "Username and list of UUIDs to assign to user", required = true) @Valid
                    TeamsSetRequest request) {
        try (QueryManager qm = new QueryManager(getAlpineRequest())) {
            return qm.callInTransaction(() -> {
                ...
                final Query<Team> query = qm.getPersistenceManager()
                        .newQuery(Team.class)
                ...
                if (requestedTeams.size() != request.teams().size()) {
                    ...
                    final List<String> differences = requestedTeams.stream()
                            .map(Team::getUuid)
                            .map(UUID::toString)
                            .toList();
                ...
                if (currentUserTeams.equals(requestedTeams)) {
                    return Response.notModified()
                            .entity("The user is already a member of the selected team(s)")
                            .build();
                }
                ...
                super.logSecurityEvent(
                        LOGGER,
                        SecurityMarkers.SECURITY_AUDIT,
                        "Added team membership for: " + principal.getUsername() + " / team: "
                                + requestedTeams.stream().map(Team::getName).toList());
                return Response.ok(principal).build();
            });
        }
    }
