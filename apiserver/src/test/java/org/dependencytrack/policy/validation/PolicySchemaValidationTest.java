...
class PolicySchemaValidationTest {
    ...
    static void beforeAll() {
        ...
        schema = SchemaRegistry.withDialect(
                        Dialects.getDraft202012(),
                        builder -> builder.nodeReader(DefaultNodeReader.builder()
                                .yamlMapper(yamlMapper)
                                .build()))
                .getSchema(PolicySchemaValidationTest.class.getResourceAsStream(
                        "/schema/vulnerability-policy-v1.schema.json"));
    }

    @Test
    void testValidPolicyYamlWithSchema() throws IOException {
        final String policyContent =
                resourceToString("/unit/policy/vulnerability-policy-v1-valid.yaml", StandardCharsets.UTF_8);
    ...
    void testInvalidPolicyYamlWithSchema() throws IOException {
        final String policyContent =
                resourceToString("/unit/policy/vulnerability-policy-v1-invalid.yaml", StandardCharsets.UTF_8);
        JsonNode jsonNode = yamlMapper.readTree(policyContent);
        List<Error> errors = schema.validate(jsonNode);
        assertThat(errors)
