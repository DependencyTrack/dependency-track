...
class ServiceAccountsResourceTest extends ResourceTest {
    ...
    @ParameterizedTest
    @CsvSource({"svc:foo, 400", "SVC:foo, 400", "svc, 201", "svcfoo, 201", "svc_foo, 201"})
    ...
    void createServiceAccountShouldAcceptCommonServiceAccountNameCharacters(String name) {
        initializeWithPermissions(Permissions.ACCESS_MANAGEMENT_CREATE, Permissions.ACCESS_MANAGEMENT_READ);

        final Response createResponse = jersey.target("/service-accounts")
                .request()
                .header(X_API_KEY, apiKey)
                .post(Entity.json(/* language=JSON */ """
                        {
        ...
        final Response getResponse = jersey.target("/service-accounts")
                .path(name)
                .request()
                .header(X_API_KEY, apiKey)
                .get();
        assertThat(getResponse.getStatus()).isEqualTo(200);
        assertThatJson(getPlainTextBody(getResponse)).isObject().containsEntry("username", "svc:" + name);
    }

    @ParameterizedTest
    @ValueSource(strings = {".pipeline", "@pipeline", "ci pipeline", "ci/pipeline", "ci$", ":pipeline"})
    void createServiceAccountShouldRejectNamesWithInvalidCharacters(String name) {
        initializeWithPermissions(Permissions.ACCESS_MANAGEMENT_CREATE);
