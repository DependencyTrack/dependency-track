...
Users do not have these problems. Since [ADR 006], all of them live in the `USER` table with a `TYPE` discriminator.
...
The prefix only helps if directory users do not already have it. We first considered `svc-`,
but [Microsoft recommends exactly that prefix for service accounts in Active Directory][microsoft-svc-accounts].
Real directory accounts named like that would have been locked out.

We chose the colon instead. [Active Directory does not allow it in the `sAMAccountName` logon name][microsoft-samaccountname].
Email addresses only allow it inside quotes, so it is rare in user principal names and OIDC usernames,
which usually follow the email format. A colon requires no encoding in a URL path,
and shells do not treat it as a special character.
...
New endpoints are added to API v2. They require `ACCESS_MANAGEMENT`,
or the matching `ACCESS_MANAGEMENT_CREATE`, `_READ`, `_UPDATE`, or `_DELETE` permission.
...
* A colon is rare in directory and OIDC usernames, but an identity provider can still send one.
  A person or directory account named `svc:backup` cannot log in. The API rejects such users with `400`,
  and logins through LDAP or OIDC fail for them. If such a user already exists, the upgrade stops
  and names the affected users. We accept this as rare.
...
[microsoft-samaccountname]: https://learn.microsoft.com/en-us/windows/win32/adschema/a-samaccountname
[microsoft-svc-accounts]: https://learn.microsoft.com/en-us/entra/architecture/service-accounts-on-premises
