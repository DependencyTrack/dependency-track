...
COPY ./target/${WAR_FILENAME} ./app.jar

RUN unzip -qq ./app.jar -d ./app \
    && JAVA_MODULE_DEPS="$(jdeps -cp './app:./app/WEB-INF/lib/*' --print-module-deps --multi-release 21 --ignore-missing-deps ./app/WEB-INF/classes)" \
    && jlink --compress zip-6 --strip-debug --no-header-files --no-man-pages --add-modules "jdk.crypto.ec,jdk.zipfs,${JAVA_MODULE_DEPS}" --output ./jre
