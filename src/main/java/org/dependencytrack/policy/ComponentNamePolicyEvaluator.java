...
/**
 * Evaluates a components name against a policy.
 *
 * @author Steve Springett
 * @since 4.0.0
 */
public class ComponentNamePolicyEvaluator extends AbstractPolicyEvaluator {
    ...
    public PolicyCondition.Subject supportedSubject() {
        return PolicyCondition.Subject.COMPONENT_NAME;
    }
    ...
    public Optional<PolicyConditionViolation> evaluate(final Policy policy, final Component component) {
        ...
        for (final PolicyCondition condition: super.extractSupportedConditions(policy)) {
            if (PolicyCondition.Operator.MATCHES == condition.getOperator()) {
                if (component.getName().contains(condition.getValue())) {
