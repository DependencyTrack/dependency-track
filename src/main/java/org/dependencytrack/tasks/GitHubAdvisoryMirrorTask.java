...
//                    })
    ...
    public void inform(final Event e) {
        ...
        try (final GitHubSecurityAdvisoryClient client = createApiClient(apiUrl, accessToken, lastModifiedEpochSeconds)) {
            ...
            if (numMirrored > 0) {
                LOGGER.info("Successfully mirrored %d GHSAs in %s (last committed modification timestamp: %s)".formatted(
            ...
            LOGGER.error("An unexpected error occurred after mirroring %d GHSAs in %s (last committed modification timestamp: %s)".formatted(
    ...
    private static final class HttpRequestRetryStrategy extends DefaultHttpRequestRetryStrategy {
        ...
        private record RateLimitInfo(
            ...
            private static RateLimitInfo of(final HttpResponse response) {
                ...
                if (retryAfterHeader != null) {
                    ...
                    return new RateLimitInfo(Duration.ofSeconds(retryAfterSeconds), null, null, null);
                }
                ...
                if (remainingRequestsHeader != null) {
                    ...
                    return new RateLimitInfo(null, remainingRequests, requestLimit, Instant.ofEpochSecond(requestLimitResetEpochSeconds));
                }
        ...
        public boolean retryRequest(final HttpResponse response, final int execCount, final HttpContext context) {
            ...
            } else if (rateLimitInfo.remainingRequests() != null) {
        ...
        public TimeValue getRetryInterval(final HttpResponse response, final int execCount, final HttpContext context) {
            ...
            if (rateLimitInfo.retryAfter() != null) {
                ...
                return TimeValue.ofMilliseconds(rateLimitInfo.retryAfter().toMillis());
            } else if (rateLimitInfo.requestLimitResetAt() != null) {
                ...
                return TimeValue.ofMilliseconds(retryAfter.toMillis());
            }
