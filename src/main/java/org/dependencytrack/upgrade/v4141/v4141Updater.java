 ...
 */
package org.dependencytrack.upgrade.v4141;
...
public class v4141Updater extends AbstractUpgradeItem {

    private static final Logger LOGGER = Logger.getLogger(v4141Updater.class);
    ...
    private void addVarcharColumnIfMissing(final Connection connection, final String tableName,
                                            final String columnName, final int length) throws SQLException {
        if (columnExists(connection, tableName, columnName)) {
            return;
        }
        DbUtil.executeUpdate(connection,
                "ALTER TABLE \"" + tableName + "\" ADD \"" + columnName + "\" VARCHAR(" + length + ")");
    }

    private boolean columnExists(final Connection connection, final String tableName,
