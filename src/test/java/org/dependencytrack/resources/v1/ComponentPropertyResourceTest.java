...
public class ComponentPropertyResourceTest extends ResourceTest {
    ...
    public void getPropertiesTest() {
		...
		temp.setName("UI");
		temp.setClassifier(Classifier.APPLICATION);
		temp.setProject(project);
		Component component = qm.createComponent(temp,true);
        qm.createComponentProperty(component, "mygroup", "prop1", "value1", IConfigProperty.PropertyType.STRING, "Test Property 1");
        qm.createComponentProperty(component, "mygroup", "prop2", "value2", IConfigProperty.PropertyType.ENCRYPTEDSTRING, "Test Property 2");
        Response response = target(V1_COMPONENT + "/" + component.getUuid().toString() + "/property").request()
    ...
    public void getPropertiesInvalidTest() {
       Response response = target(V1_COMPONENT + "/" + UUID.randomUUID().toString() + "/property").request()
                .header(X_API_KEY, apiKey)
                .get(Response.class);
    ...
    public void createPropertyTest() {
        ...
        Response response = target(V1_COMPONENT + "/" + project.getUuid().toString() + "/property").request()
                .header(X_API_KEY, apiKey)
    ...
    public void createPropertyDuplicateTest() {
		...
		temp.setProject(project);
		Component component = qm.createComponent(temp,true);
		qm.createComponentProperty(component, "mygroup", "prop1", "value1", IConfigProperty.PropertyType.STRING, null);
        ...
        property.setPropertyType(IConfigProperty.PropertyType.STRING);
        property.setDescription("Test Property 1");
        Response response = target(V1_COMPONENT + "/" + uuid + "/property").request()
                .header(X_API_KEY, apiKey)
                .put(Entity.entity(property, MediaType.APPLICATION_JSON));
        Assert.assertEquals(409, response.getStatus(), 0);
        Assert.assertNull(response.getHeaderString(TOTAL_COUNT_HEADER));
        String body = getPlainTextBody(response);
        Assert.assertEquals("A property with the specified component/group/name combination already exists.", body);
    }
    ...
    public void createPropertyInvalidTest() {
        ...
        property.setPropertyType(IConfigProperty.PropertyType.STRING);
        property.setDescription("Test Property 1");
        Response response = target(V1_COMPONENT + "/" + UUID.randomUUID() + "/property").request()
                .header(X_API_KEY, apiKey)
                .put(Entity.entity(property, MediaType.APPLICATION_JSON));
        Assert.assertEquals(404, response.getStatus(), 0);
        Assert.assertNull(response.getHeaderString(TOTAL_COUNT_HEADER));
    ...
    public void updatePropertyTest() {
		...
		temp.setProject(project);
		Component component = qm.createComponent(temp,true);
        ComponentProperty property = qm.createComponentProperty(component, "mygroup", "prop1", "value1", IConfigProperty.PropertyType.STRING, null);
