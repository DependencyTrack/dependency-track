...
public class VexUploadProcessingTaskTest extends PersistenceCapableTest {
    ...
    @Test
    void loggerMessagesContainUuid() throws Exception {
        final Logger logger = (Logger) LoggerFactory.getLogger(VexUploadProcessingTask.class);
            ...
            await().atMost(Duration.ofSeconds(5)).untilAsserted(() -> {
                assertThat(appender.list).anySatisfy(event -> {
                    assertThat(event.getMDCPropertyMap())
                            .containsEntry(MDC_PROJECT_UUID, project.getUuid().toString());
                });
            });
