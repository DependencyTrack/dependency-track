...
/**
 * Exercises probe_skipped_users population and the downstream-invalidation rule on re-extract.
 */
@TestInstance(TestInstance.Lifecycle.PER_CLASS)
class InvalidationAndProbesIT {
    ...
    void nullUsernameRowsLandInProbeAndReRunInvalidatesDownstream() throws Exception {
        // Seed a NULL-USERNAME LDAP row and a normal MANAGED row.
        source.jdbi().useHandle(h -> {
            h.execute("""
                ...
                VALUES (1, 'alice', 'h', FALSE, '2025-01-01T00:00:00Z', FALSE, FALSE)
                """);
            h.execute("""
